...
class Toc {
  ...
  __updateList() {
    ...
    return (() => {
      ...
      for (let i in this.lines) {
        ...
        if (result) {
          const depthFrom = this.options.depthFrom !== undefined ? this.options.depthFrom : 1;
          const depthTo = this.options.depthTo !== undefined ? this.options.depthTo : 6;
          if ((result[0].length <= parseInt(depthTo)) && (result[0].length >= parseInt(depthFrom))) {
            result1.push(this.list.push({
  ...
  __createList() {
    ...
    const depthFrom = this.options.depthFrom !== undefined ? this.options.depthFrom : 1;
    const depthTo = this.options.depthTo !== undefined ? this.options.depthTo : 6;
    let indicesOfDepth = Array.apply(null, new Array((depthTo - depthFrom) + 1)).map(Number.prototype.valueOf, 0);
    for (let i of Object.keys(this.list || {})) {
      var item = this.list[i];
      const row = [];
      for (let tab = depthFrom, end = item.depth, asc = depthFrom <= end; asc ? tab <= end : tab >= end; asc ? tab++ : tab--) {
        if (tab > depthFrom) {
          row.push("\t");
        }
      }
      if (this.options.orderedList === 1) {
        ...
        indicesOfDepth = indicesOfDepth.map(function(value, index) { if (index < item.depth) { return value; } else { return 0; } });
      } else {
        row.push("- ");
      }
